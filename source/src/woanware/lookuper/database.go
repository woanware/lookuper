...
func resetTables(displayMessage bool) {
	...
	_, err = db.Exec("DROP TABLE job;")
	if err != nil {
		log.Printf("Error dropping job table: %v", err)
		return
	}

	_, err = db.Exec(SQL_CREATE_TABLE_JOB)
	if err != nil {
		log.Printf("Error creating job table: %v", err)
		return
	}
